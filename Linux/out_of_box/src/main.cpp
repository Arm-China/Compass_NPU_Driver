...
int main(int argc, char *argv[]) {
  ...
  if (input_desc.size() != input_data.size()) {
    fprintf(stderr, "[TEST ERROR] input data files number doesn't match with "
                    "input tensor number\n");
    goto unload_graph;
  }
  ...
  fprintf(stdout,
          "[TEST INFO] aipu_get_tensor_count success: output cnt = %d\n",
          output_cnt);
