...
#
# sim_dyshape.py:
# 	this script is for running a model which supports to update input shape dynamically on Simualtor.
#
# usage:
...
# "simulator" just for aipu v1/v2
global_cfg = {
	"simulator" : parseCmdline_obj.m_emulator_path,
	"log_file_path" : parseCmdline_obj.m_dump_path,
	"log_level" : "0",
	"verbose" : "0",
	"enable_avx" : "0",
	"enable_calloc" : "0",
...
if ret != AIPU_STATUS_SUCCESS:
    errmsg = npu.aipu_get_error_message(ret)
    log.error(f'aipu_config_global [fail], err: {errmsg}')
    exit(-1)
else:
...
for benchmark in parseCmdline_obj.m_benchmarks_list:
	...
	retmap = npu.aipu_create_job(graph_id, job_cfg, fm_idxes, parseCmdline_obj.m_input_shape)
	if retmap["ret"] != AIPU_STATUS_SUCCESS:
		errmsg = npu.aipu_get_error_message(retmap["ret"])
	...
	if os.path.isdir(parseCmdline_obj.m_dump_path):
		...
		ret = npu.aipu_config_job(job_id, AIPU_CONFIG_TYPE_SIMULATION, job_cfg_dump)
		if ret != AIPU_STATUS_SUCCESS:
			errmsg = npu.aipu_get_error_message(ret)
			log.error(f'aipu_config_job [fail], err: {errmsg}')
			npu.aipu_clean_job(job_id)
			...
			log.debug(f'aipu_config_job [ok]')
	...
	if LOAD_FROM_FILE_FLAG: # load 2-1
		for i in range(len(input_bins)):
			...
			if ret != AIPU_STATUS_SUCCESS:
				errmsg = npu.aipu_get_error_message(ret)
				log.error(f'aipu_load_tensor file [fail], err: {errmsg}')
				npu.aipu_clean_job(job_id)
				npu.aipu_unload_graph(graph_id)
				npu.aipu_deinit_context()
				exit(-1)
	...
	if ret != AIPU_STATUS_SUCCESS:
		errmsg = npu.aipu_get_error_message(ret)
		log.error(f'aipu_finish_job [fail], err: {errmsg}')
		npu.aipu_clean_job(job_id)
		npu.aipu_unload_graph(graph_id)
		npu.aipu_deinit_context()
		exit(-1)
	else:
		log.debug(f'aipu_finish_job [ok]')
	...
	for i in range(output_cnt):
		...
		if retmap["ret"][0] != AIPU_STATUS_SUCCESS: # note the return value checking for this API
			...
			log.error(f'aipu_get_tensor [fail], err: {errmsg}')
			npu.aipu_clean_job(job_id)
			npu.aipu_unload_graph(graph_id)
			npu.aipu_deinit_context()
			exit(-1)
