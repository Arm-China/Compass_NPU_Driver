...
if [ "$PLATFORM"x == "arm64"x ]; then
    if [ "$ARCH" != "X2" ]; then
        MAKE_FLAG="CXX=aarch64-linux-gnu-g++ CPPFLAGS+=-DZHOUYI_V12=1 BUILD_AIPU_VERSION=aipu_v1v2 BUILD_TARGET_PLATFORM=fpga"
    else
        MAKE_FLAG="CXX=aarch64-linux-gnu-g++ CPPFLAGS+=-DZHOUYI_V3=1 BUILD_AIPU_VERSION=aipu_v3 BUILD_TARGET_PLATFORM=fpga"
    fi
else
    if [ "$ARCH" != "X2" ]; then
        MAKE_FLAG="CXX=g++ CPPFLAGS+=-DSIMULATION=1 CPPFLAGS+=-DZHOUYI_V12=1 BUILD_AIPU_VERSION=aipu_v1v2 BUILD_TARGET_PLATFORM=sim"
    else
        MAKE_FLAG="CXX=g++ CPPFLAGS+=-DSIMULATION=1 CPPFLAGS+=-DZHOUYI_V3=1 BUILD_AIPU_VERSION=aipu_v3 BUILD_TARGET_PLATFORM=sim"
    fi
