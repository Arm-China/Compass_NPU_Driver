...
# for x2/x3p
$ ./aipu_simulation_test -b aipu.bin -i input0.bin -c output.bin -d ./
```

note:
- aipu.bin is generated by specific buildtool(NN-Compiler).
...
If AIPU arch is v3 or above, you should provide target by '-a'
...
- multiple_bss_test: multiple weight files test for large model
```bash
...
```bash
$ aipugb graph.def -w weight.bin --disable_input_buffer_reuse --disable_output_buffer_reuse --target X2_1204 -o aipu.bin
```