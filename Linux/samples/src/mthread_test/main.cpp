 ...
 *
 * @note
 *        non-pipeline: aipu_mthread_test -b aipu.bin -i input.bin -c output.bin
 * -d ./output/ pipeline: aipu_mthread_test -p -b aipu.bin -i input.bin -c
 * output.bin -d ./output/
 *
 *        support running both on HW and Simulator, note that it has to insmod
 * aipu.ko firstly on HW platform.
 */
...
void non_pipeline(uint32_t frame_cnt) {
  ...
  path = std::string(opt.dump_dir) + "/" + std::to_string(gettid());
  if (opt.dump_dir[0] != '\0') {
    ...
    AIPU_INFO() << "dump_dir: " << mem_dump_config.dump_dir;
  ...
  if (run_on_platform == ON_SIMULATOR) {
    help_create_dir(path.c_str());
    sim_job_config.data_dir = path.c_str();
...
void pipeline(uint32_t pipe_cnt) {
  aipu_status_t ret = AIPU_STATUS_SUCCESS;
  aipu_create_job_cfg_t create_job_cfg = {0};
  const char *msg = nullptr;
  uint64_t graph_id;
  ...
  for (uint32_t frame = 0; frame < pipe_cnt; frame++) {
    ...
    if (opt.dump_dir[0] != '\0') {
      ...
      uint64_t cfg_types =
          ...
          AIPU_JOB_CONFIG_TYPE_DUMP_EMULATION;
      help_create_dir(mem_dump_config.dump_dir);
      AIPU_INFO() << mem_dump_config.dump_dir;
      ret =
          aipu_config_job(ctx, job_id_vec[frame], cfg_types, &mem_dump_config);
      if (ret != AIPU_STATUS_SUCCESS) {
        aipu_get_error_message(ctx, ret, &msg);
        AIPU_ERR()("aipu_config_job: %s\n", msg);
        goto clean_job;
  ...
  for (uint32_t frame = 0; frame < pipe_cnt; frame++) {
    ...
    if (aipu_sts == AIPU_STATUS_SUCCESS) {
      if (aipu_job_sts == AIPU_JOB_STATUS_DONE) {
        ...
        result =
            check_result(output_data_vec, output_desc, opt.gts, opt.gts_size);
        if (result != 0)
          pass = result;
      ...
      AIPU_ERR()
      ("get_job_status (%lx): status=%x, job_status=%x [fail]\n",
...
int main(int argc, char *argv[]) {
  ...
  AIPU_CRIT() << "usage: ./aipu_mthread_test [-p] -b aipu.bin -i input0.bin -c "
                 "output.bin -d ./\n";
