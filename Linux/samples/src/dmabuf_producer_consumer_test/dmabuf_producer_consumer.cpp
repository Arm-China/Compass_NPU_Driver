...
/**
 * @brief get a dmabuf from producer and set it as input tensor, as well as
 * request a dmabuf as output tensor, then infer the model, finally it transfer
 * output dmabuf to consumer which will check the inferrence result.
 *
...
int main(int argc, char *argv[]) {
  ...
  for (loop = 0; loop < total_loop; loop++) {
    ...
    // dmabuf_dump_file("dmabuf_input.bin", dmabuf_input_fd,
    // input_desc[0].size);
