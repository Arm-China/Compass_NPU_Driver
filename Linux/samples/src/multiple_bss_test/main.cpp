...
/**
 * @file  main.cpp
 * @brief aipu v3/v3_1 AIPU UMD test application: multiple weight files test for
 * LLM
 *
 * @note  it should push all extra weight files into one directory, the weight
 * names are like extra_weight_{0-9}.bin, don't rename them. then specify the
 * top directory of extra weight files with command parameter '-w dir'.
 *
 *        /opt/resnet_model/
 *        |-- aipu.bin
 *        |-- extra_weight_0.bin
 ...
 *        |-- output.bin
 *
 *        - ./aipu_multiple_bss_test -b /opt/resnet_model/aipu.bin -i
 * /opt/resnet_model/input0.bin -c /opt/resnet_model/output.bin -w
 * /opt/resnet_model/ -d ./
 *
...
int main(int argc, char *argv[]) {
  ...
  AIPU_CRIT() << "usage: ./aipu_multiple_bss_test -b aipu.bin -i input0.bin -c "
                 "output.bin "
                 "-w extra_wight_dir -d ./\n";
