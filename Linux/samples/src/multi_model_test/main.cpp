...
void pipeline() {
  ...
  for (uint32_t i = 0; i < GRAPH_NUM; i++) {
    ...
    infer_info[i].inputbins.assign(opt.input_files.begin(),
                                   opt.input_files.end());
    ...
    for (uint32_t j = 0; j < infer_info[i].in_tensor_cnt; j++) {
      ...
      ret = aipu_get_tensor_descriptor(ctx, graph_id, AIPU_TENSOR_TYPE_INPUT, j,
                                       &desc);
      if (ret != AIPU_STATUS_SUCCESS) {
        aipu_get_error_message(ctx, ret, &msg);
    ...
    for (uint32_t k = 0; k < infer_info[i].out_tensor_cnt; k++) {
      ...
      shared_ptr<char> sp(new char[infer_info[i].out_tensor_desc[k].size],
                          deleter);
  ...
  /**
   * NOTE:
   *       load the coressponding input frame data for each job and commit the
   * jobs.
   */
  for (uint32_t i = 0; i < GRAPH_NUM; i++) {
    AIPU_INFO()("Graph #%u\n", i);
    for (uint32_t j = 0; j < infer_info[i].in_tensor_cnt; j++) {
      ret = aipu_load_tensor(ctx, infer_info[i].job_id, j,
                             infer_info[i].input_data_vec[j]);
      if (ret != AIPU_STATUS_SUCCESS) {
        aipu_get_error_message(ctx, ret, &msg);
      ...
      ("load input tensor %d from %s (%u/%u)\n", j,
       infer_info[i].inputbins[j].c_str(), j + 1, infer_info[i].in_tensor_cnt);
    }
  ...
  /**
   * NOTE:
   *      poll each job's status, if job is done normally, check its output
   * data.
   */
  for (uint32_t i = 0; i < GRAPH_NUM; i++) {
    ...
    while ((aipu_job_sts != AIPU_JOB_STATUS_DONE) &&
      ...
      aipu_sts =
          aipu_get_job_status(ctx, infer_info[i].job_id, &aipu_job_sts, 5);
      ...
      if (aipu_job_sts == AIPU_JOB_STATUS_DONE) {
        for (uint32_t j = 0; j < infer_info[i].out_tensor_cnt; j++) {
          ret = aipu_get_tensor(ctx, infer_info[i].job_id,
                                ...
                                infer_info[i].output_data_vec[j].get());
          if (ret != AIPU_STATUS_SUCCESS) {
            aipu_get_error_message(ctx, ret, &msg);
...
int main(int argc, char *argv[]) {
  ...
  AIPU_CRIT() << "usage: ./aipu_multi_model_test -b aipu.bin -i input0.bin -c "
                 "output.bin -d ./\n";
