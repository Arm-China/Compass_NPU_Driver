...
/**
 * @file  main.cpp
 * @brief AIPU UMD test application: dmabuf attach test
 */

/**
 * @brief accept a dma-buf's fd and attach it, then
 *        specify this dma_buf as model's input tensor buffer.
 *        based on this demo, you can specify the dma-buf as
...
/**
 * 0: AIPU_SHARE_BUF_ATTACH_DMABUF, attach dma-buf case
...
/**
 * request one dma_buf from dma_buf exporter(simulate external module with NPU
 * driver), record its fd to 'dmabuf_fd'.
 */
...
int main(int argc, char *argv[]) {
  ...
  for (loop = 0; loop < total_loop; loop++) {
    ...
    share_tensor.shared_case_type = AIPU_SHARE_BUF_ATTACH_DMABUF;
#endif
