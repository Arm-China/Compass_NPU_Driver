...
/**
 * @file  main.cpp
 * @brief AIPU UMD test application: test for aipu_load_graph_helper on arm64
 * platforms
 */
...
int debugger_bind_job_x2(aipu_ctx_handle_t *m_ctx_handle, uint64_t m_job_id) {
  ...
  if (AIPU_STATUS_SUCCESS != result) {
    aipu_get_error_message(m_ctx_handle, result, &status_msg);
    AIPU_ERR()("RT_API:aipu_get_partition_count: %s.", status_msg);
    return error;
  }
  if (aipu_part_count < 1) {
    aipu_get_error_message(m_ctx_handle, result, &status_msg);
    AIPU_ERR()
    ("RT_API:aipu_get_partition_count: part_count: %d", aipu_part_count);
    return error;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (opt.loop_cnt != 0)
    AIPU_CRIT()
    ("aipu_debugger_test doesn't support to specify outer loop counter\n");

  if (opt.frame_cnt != 0)
    frame_cnt = opt.frame_cnt;

  if (opt.dump_dir[0] != '\0')
    AIPU_CRIT()("aipu_debugger_test doesn't support to dump files\n");
