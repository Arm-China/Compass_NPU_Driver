 ...
 *
 * @note
 *        1, run same model(eg: alexnet) with only one input tensor in loop-0
 * and loop-1 2, loop == 0: mark one input tensor buffer of graph-0 as shared
...
int main(int argc, char *argv[]) {
  ...
  AIPU_CRIT()
      << "usage: ./aipu_time_cost_test -t flush|finish -b aipu.bin,aipu.bin... "
         "-i input0.bin -c output.bin -d ./\n"
      << "  1.all aipu.bins must be same, aipu.bin number represents number of "
         "loading graphs\n"
      << "  2.input.bins and output.bins only need one copy to aipu.bin\n";
  ...
  for (uint32_t loop = 0; loop < len; loop++) {
    ...
    ret = aipu_get_tensor_count(ctx, graph_id[loop], AIPU_TENSOR_TYPE_INPUT,
                                &input_cnt);
    if (ret != AIPU_STATUS_SUCCESS) {
      aipu_get_error_message(ctx, ret, &msg);
    ...
    for (uint32_t i = 0; i < input_cnt; i++) {
      ...
      ret = aipu_get_tensor_descriptor(ctx, graph_id[loop],
                                       AIPU_TENSOR_TYPE_INPUT, i, &desc);
      if (ret != AIPU_STATUS_SUCCESS) {
        aipu_get_error_message(ctx, ret, &msg);
    ...
    ret = aipu_get_tensor_count(ctx, graph_id[loop], AIPU_TENSOR_TYPE_OUTPUT,
                                &output_cnt);
    if (ret != AIPU_STATUS_SUCCESS) {
      aipu_get_error_message(ctx, ret, &msg);
      AIPU_ERR()("aipu_get_tensor_count: %s\n", msg);
    ...
    create_dur = (create_t2.tv_sec - create_t1.tv_sec) * 1000000 +
                 (create_t2.tv_usec - create_t1.tv_usec);
    AIPU_INFO()("create grpah<%d>, job<%d>: %d us\n", loop, loop, create_dur);
    create_total += create_dur;
    AIPU_INFO()
    ("total %d graphs, create %d jobs: %d us\n", loop + 1, loop + 1,
     create_total);
    ...
    for (uint32_t i = 0; i < input_cnt; i++) {
      if (input_desc[i].size > opt.inputs_size[input_file_idx]) {
        ...
        ("input file %s len 0x%x < input tensor %u size 0x%x\n",
         opt.input_files[i].c_str(), opt.inputs_size[input_file_idx], i,
         input_desc[i].size);
        goto clean_job;
      }
      ...
      ("load input tensor %d from %s (%u/%u)\n", i,
       opt.input_files[input_file_idx].c_str(), i + 1, input_cnt);
  ...
  if (flush_time) {
    ...
    for (uint32_t job = 0; job < len; job++) {
      ...
      if (run_on_platform == ON_SIMULATOR) {
        ret = aipu_config_job(ctx, job_id[job], AIPU_CONFIG_TYPE_SIMULATION,
                              &sim_job_config);
        if (ret != AIPU_STATUS_SUCCESS) {
          aipu_get_error_message(ctx, ret, &msg);
          AIPU_ERR()("aipu_config_job: %s\n", msg);
      ...
      flush_dur = (flush_t2.tv_sec - flush_t1.tv_sec) * 1000000 +
                  (flush_t2.tv_usec - flush_t1.tv_usec);
    ...
    pre_total = (pre_t2.tv_sec - pre_t1.tv_sec) * 1000000 +
                (pre_t2.tv_usec - pre_t1.tv_usec);
    ...
    for (uint32_t job = 0; job < len; job++) {
      ...
      finish_dur = (finish_t2.tv_sec - finish_t1.tv_sec) * 1000000 +
                   (finish_t2.tv_usec - finish_t1.tv_usec);
      ...
      for (uint32_t i = 0; i < output_desc_vec[job].size(); i++) {
        memset(job_outputs[job][i], 0, (output_desc_vec[job])[i].size);
        ret = aipu_get_tensor(ctx, job_id[job], AIPU_TENSOR_TYPE_OUTPUT, i,
                              job_outputs[job][i]);
        if (ret != AIPU_STATUS_SUCCESS) {
          aipu_get_error_message(ctx, ret, &msg);
          AIPU_ERR()("aipu_get_tensor: %s\n", msg);
        ...
        AIPU_INFO()
        ("job %d: get output tensor %u success (%u/%u)\n", job, i, i + 1,
         output_cnt);
      }
