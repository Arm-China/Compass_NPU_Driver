...
{
	if (likely(core))
		aipu_write32(core->reg, ZHOUYI_CTRL_REG_OFFSET, ZHOUYI_V2_IRQ_DISABLE_FLAG);
}
...
static int zhouyi_v2_reserve(struct aipu_partition *core, struct aipu_job_desc *udesc,
			     int do_trigger, int pool)
{
	...
	enable_dtcm = udesc->dtcm_size_kb ? ZHOUYI_V2_DTCM_ENABLE : 0;
...
static void zhouyi_v2_print_hw_id_info(struct aipu_partition *core)
{
	...
	is_v2_2 = core->version == AIPU_ISA_VERSION_ZHOUYI_V2_2;
	...
	dev_info(core->dev, "########## AIPU CORE %d: ZHOUYI %s%d ##########",
		 core->id, is_v2_2 ? "X" : "Z", is_v2_2 ? 1 : core->version);
...
static int zhouyi_get_max_reg_offset(struct aipu_partition *core)
{
	...
	if (core->version == AIPU_ISA_VERSION_ZHOUYI_V2_2)
...
static int zhouyi_v2_io_rw(struct aipu_partition *core, struct aipu_io_req *io_req)
{
	...
	if (io_req->offset == ZHOUYI_V2_SOFT_RESET_OFFSET &&
...
static void zhouyi_v2_initialize(struct aipu_partition *core)
{
	...
	if (core->dtcm_size > 0 && core->version == AIPU_ISA_VERSION_ZHOUYI_V2_2) {
		...
		aipu_write32(core->reg, ZHOUYI_V2_DTCM_HIGH_BASE_ADDR, core->dtcm_base >> 32);
		aipu_write32(core->reg, ZHOUYI_V2_DTCM_LOW_BASE_ADDR, (u32)core->dtcm_base);
	}
}
...
static int zhouyi_soft_reset_no_lock(struct aipu_partition *core, bool init_regs)
{
	...
	if (core->version == AIPU_ISA_VERSION_ZHOUYI_V2_2)
		ret = zhouyi_soft_reset(core->reg, ZHOUYI_V2_SOFT_RESET_OFFSET,
					core->reset_delay_us);
...
static int zhouyi_v2_destroy_command_pool(struct aipu_partition *core, int pool)
{
	return 0;
}

static int zhouyi_v2_abort_command_pool(struct aipu_partition *core, int pool)
{
	return 0;
}
