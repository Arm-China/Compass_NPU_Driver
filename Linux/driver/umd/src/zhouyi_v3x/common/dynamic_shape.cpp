...
aipu_status_t
DynamicShape::update_dynamic_io_tensor_size(aipu_tensor_type_t type) {
...
bool DynamicShape::set_dynamic_shape_data(aipu_dynshape_param_t *shape_param) {
  ...
  for (uint32_t idx = 0; idx < shape_constraint.size(); ++idx) {
    ...
    if (shape_param != nullptr) {
      for (uint32_t item_idx = 0; item_idx < shape_param->input_shape_cnt;
           ++item_idx) {
    ...
    if (shape_item == nullptr) {
      for (uint32_t dim = 0; dim < shape_constraint.at(idx)[0].size(); ++dim)
        m_config_in_tensor_shape[idx].push_back(
            shape_constraint.at(idx)[0][dim]);

      uint32_t size = std::accumulate(m_config_in_tensor_shape[idx].begin(),
                                      m_config_in_tensor_shape[idx].end(), 1,
