...
private:
  ...
  aipu_status_t setup_dynamic_shape_buffer();
  aipu_status_t setup_ws_buffer();
...
protected:
  ...
  virtual aipu_status_t alloc_load_job_buffers() override;
  virtual aipu_status_t alloc_reuse_buffer() override;
  virtual aipu_status_t free_job_buffers() override;

  virtual aipu_status_t init_group_id(uint32_t sg_cnt) {
    return AIPU_STATUS_SUCCESS;
  }
  virtual void free_sg_buffers(SubGraphTask &sg_task);
  aipu_status_t
  setup_rodata_sg(const std::vector<struct GraphParamMapLoadDesc> &param_map,
