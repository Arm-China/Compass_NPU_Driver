...
void aipudrv::GraphV3X::print_parse_info()
{
    ...
    LOG(LOG_DEFAULT, "\n--Weight:    cnt  0x%lx", m_bweight.size());
    for (uint32_t i = 0; i < m_bweight.size(); i++)
    {
        LOG(LOG_DEFAULT, "[Weight #%d]:    size 0x%lx", i, m_bweight[i].size);
    }
    ...
    for (uint32_t i = 0; i < get_bss_cnt(); i++)
    {
        ...
        LOG(LOG_DEFAULT, "--static:     cnt 0x%lx", get_bss(i).static_sections.size());
        for (uint32_t j = 0; j < get_bss(i).static_sections.size(); j++)
        {
            LOG(LOG_DEFAULT, "----static section [%d]: size 0x%x, align 0x%x",
                j, get_bss(i).static_sections[j].size, get_bss(i).static_sections[j].align_in_page);
            for (uint32_t k = 0; k < get_bss(i).static_sections[j].sub_sections.size(); k++)
            {
                LOG(LOG_DEFAULT, "------subsection [%d]: offset 0x%x",
                    k, get_bss(i).static_sections[j].sub_sections[k].offset_in_section);
            }
        }

        LOG(LOG_DEFAULT, "--reuse:      cnt 0x%lx", get_bss(i).reuse_sections.size());
        for (uint32_t j = 0; j < get_bss(i).reuse_sections.size(); j++)
        {
            LOG(LOG_DEFAULT, "----reuse section [%d]: size 0x%x, align 0x%x",
                j, get_bss(i).reuse_sections[j].size, get_bss(i).reuse_sections[j].align_in_page);
            for (uint32_t k = 0; k < get_bss(i).reuse_sections[j].sub_sections.size(); k++)
            {
                LOG(LOG_DEFAULT, "------subsection [%d]: offset 0x%x",
                    k, get_bss(i).reuse_sections[j].sub_sections[k].offset_in_section);
            }
        }
    }
    LOG(LOG_DEFAULT, "============================================================");
...
aipu_status_t aipudrv::GraphV3X::extract_gm_info(int sg_id)
{
    ...
    for (uint32_t i = 0; i < gm_buffer_cnt; i++)
    {
        ...
        {
            for (auto item : get_bss(sg_id).io.inputs)
            {
                if (gmconfig->GM_buf_idx[i].buf_index == item.ref_section_iter)
                {
                    ...
                    for (auto out_item : get_bss(sg_id).io.outputs)
                    {
            ...
            for (auto item : get_bss(sg_id).io.outputs)
            {
                if (gmconfig->GM_buf_idx[i].buf_index == item.ref_section_iter)
                {
             ...
             */
            if (gmconfig->GM_buf_idx[i].buf_index < get_bss(sg_id).reuse_sections.size())
                gm_info_desc.gm_buf_type = GM_SUB_BUF_TYPE_TEMP;

        } else if (gmconfig->GM_buf_idx[i].buf_type == GM_BUF_TYPE_WEIGHT) {
            if (gmconfig->GM_buf_idx[i].buf_index < get_bss(sg_id).static_sections.size())
            {
