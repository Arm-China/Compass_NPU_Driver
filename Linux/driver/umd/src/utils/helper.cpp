...
aipu_status_t umd_dump_file_helper(const char *fname, const void *src,
  ...
  if (fd == -1) {
    LOG(LOG_ERR, "create bin file failed: %s! (errno = %d)", fname, errno);
...
aipu_status_t umd_load_file_helper(const char *fname, void *dest,
  ...
  if (read(fd, dest, size) < 0) {
    LOG(LOG_ERR, "load file failed: %s! (errno = %d)", fname, errno);
...
aipu_status_t umd_mmap_file_helper(const char *fname, void **data,
  ...
  if (stat(fname, &finfo) != 0) {
    LOG(LOG_ERR, "no such a file: %s! (errno = %d)", fname, errno);
    ret = AIPU_STATUS_ERROR_OPEN_FILE_FAIL;
    goto finish;
  }
...
aipu_status_t umd_mmap_file_helper(const char *fname, void **data,
  ...
  if (finfo.st_size < static_cast<int64_t>(size + offset)) {
    LOG(LOG_ERR, "mmap is out of range, file size is: %lu, access range: %lu",
        finfo.st_size, size + offset);
    ret = AIPU_STATUS_ERROR_MAP_FILE_FAIL;
    goto finish;
  }
  ...
  /* success */
  *data = (char *)p_file + offset - pa_offset;
...
void dump_stack(void) {
  ...
  address_len =
      backtrace(address_array, sizeof(address_array) / sizeof(void *));
  fprintf(stderr, "\nStack backtrace:\n");
  if (address_len == 0) {
  ...
  for (int i = 1; i < address_len; i++) {
    char *mangled_name_start = nullptr, *begin_offset = nullptr,
         *end_offset = nullptr;
    ...
    if (mangled_name_start && begin_offset && end_offset &&
      ...
      *end_offset = '\0';
      ...
      start_name = abi::__cxa_demangle(mangled_name_start, pfunc_name,
                                       &func_name_sz, &status);
      if (status == 0) {
        pfunc_name = start_name;
        fprintf(stderr, "  %s : %s+%s\n", sym_array[i], pfunc_name,
                begin_offset);
      } else {
        fprintf(stderr, "  %s : %s()+%s\n", sym_array[i], mangled_name_start,
                begin_offset);
      }
    } else {
      fprintf(stderr, "  %s\n", sym_array[i]);
