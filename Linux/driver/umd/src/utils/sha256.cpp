...
/**
 * @file  sha256.cpp
 * @brief UMD secure hash algorithm function implementation
 */
...
static void sha256_block(sha256 *sha) {
...
void sha256_append_byte(sha256 *sha, uint8_t byte) {
...
const std::vector<uint8_t> sha256_bytes(const void *src, size_t n_bytes) {
  ...
  ret.resize(SHA256_BYTES_SIZE);
  uint8_t *ptr = (uint8_t *)(&ret[0]);
  for (int i = 0; i < 8; i++) {
    for (int j = 3; j >= 0; j--) {
      *ptr++ = (sha.state[i] >> j * 8) & 0xff;
    }
