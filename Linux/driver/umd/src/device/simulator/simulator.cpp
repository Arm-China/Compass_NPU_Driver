...
/**
 * @file  simulator.cpp
 * @brief AIPU User Mode Driver (UMD) zhouyi z1/2/3 simulator module
 * implementation
 */
...
aipu_status_t Simulator::update_simulation_rtcfg(const JobDesc &job,
  ...
  if (job.aipu_revision == 0) {
    ...
    if (is_x1)
      ...
      ofs << "CONFIG=Z" << job.kdesc.aipu_version << "-" << std::setw(4)
          << std::setfill('0') << job.kdesc.aipu_config << "\n";
  } else if (job.aipu_revision == AIPU_REVISION_P) {
    ofs << "CONFIG=Z" << job.kdesc.aipu_version << "-" << std::setw(4)
        << std::setfill('0') << job.kdesc.aipu_config << "p"
        << "\n";
  }
  ...
  input_data_cnt =
      2 + dcr_cnt + weight_cnt + zerocpy_const_cnt + job.reuses.size();
  ...
  if (dcr_cnt == 1) {
    ofs << "INPUT_DATA_FILE" << std::dec << input_file_idx << "=" << ctx.dcr
        << "\n";
    ofs << "INPUT_DATA_BASE" << std::dec << input_file_idx << "=0x" << std::hex
        << job.dcr_pa << "\n";
    input_file_idx++;
  }
    ...
    if (job.weights != nullptr) {
      for (uint32_t i = 0; i < job.weights->size(); i++) {
        ofs << "INPUT_DATA_FILE" << std::dec << input_file_idx + i << "="
            << ctx.weights[i] << "\n";
        ofs << "INPUT_DATA_BASE" << std::dec << input_file_idx + i << "=0x"
            << std::hex << (*job.weights)[i]->pa << "\n";
      }
  ...
  for (uint32_t i = 0; i < job.reuses.size(); i++) {
    ofs << "INPUT_DATA_FILE" << std::dec << input_file_idx + i << "="
        << ctx.reuses[i] << "\n";
    ofs << "INPUT_DATA_BASE" << std::dec << input_file_idx + i << "=0x"
        << std::hex << job.reuses[i]->pa << "\n";
  }

  ofs << "\n";
  if (job.dump_reuse) {
    ofs << "OUTPUT_DATA_CNT=" << std::dec
        << (job.outputs.size() + job.misc_outputs.size() + reuse_outputs.size())
        << "\n";
  } else {
    ofs << "OUTPUT_DATA_CNT=" << std::dec
        << job.outputs.size() + job.misc_outputs.size() << "\n";
  }

  for (auto &buf : job.outputs) {
    ofs << "OUTPUT_DATA_FILE" << std::dec << output_idx << "="
        << ctx.outputs[output_idx] << "\n";
    ofs << "OUTPUT_DATA_BASE" << std::dec << output_idx << "=0x" << std::hex
        << buf.pa << "\n";
    ofs << "OUTPUT_DATA_SIZE" << std::dec << output_idx << "=0x" << std::hex
        << buf.size << "\n";
    output_idx++;
  }
