...
namespace aipudrv {
JobV12::JobV12(MainContext *ctx, GraphBase &graph, DeviceBase *dev,
...
int JobV12::alloc_reuse_buffer_optimized() {
  ...
  for (uint32_t i = 0; i < graph().m_reuse_sections.size(); i++) {
    ...
    LOG(LOG_DEBUG,
        "buf %d: align total size: %x, buf align size: %x, align_in_page:%d", i,
        reuse_buf_total_size, ALIGN_PAGE(section_desc.size), align_in_page);
  ...
  for (uint32_t i = 0; i < graph().m_reuse_sections.size(); i++) {
    ...
    if (size != 0) {
      offset =
          (offset + ((align_in_page << 12) - 1)) & ~((align_in_page << 12) - 1);
      LOG(LOG_DEBUG, "buf %d: off: %x, pa: %lx", i, offset,
  ...
  for (uint32_t i = 0; i < graph().m_reuse_sections.size(); i++) {
    ...
    if (size != 0) {
      ...
      LOG(LOG_DEBUG, "buf %d: align_in_page: %d, sz: %lx, req_sz: %lx, pa: %lx",
          i, align_in_page, bufferDesc->req_size, bufferDesc->size,
          bufferDesc->pa);
    }
...
aipu_status_t JobV12::init(const aipu_global_config_simulation_t *cfg,
...
aipu_status_t JobV12::alloc_load_job_buffers() {
  aipu_status_t ret = AIPU_STATUS_SUCCESS;
  int retval = -1;
...
aipu_status_t JobV12::free_job_buffers() {
  aipu_status_t ret = AIPU_STATUS_SUCCESS;
...
aipu_status_t JobV12::destroy() { return free_job_buffers(); }

aipu_status_t
JobV12::config_simulation(uint64_t types,
...
aipu_status_t JobV12::bind_core(uint32_t core_id) {
  aipu_status_t ret = AIPU_STATUS_SUCCESS;
...
aipu_status_t JobV12::debugger_run() {
  aipu_status_t ret = AIPU_STATUS_SUCCESS;
...
aipu_status_t JobV12::get_runtime_err_code() const {
  if (m_err_code.size() > 0) {
    uint32_t err = 0;
