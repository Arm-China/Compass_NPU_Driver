...
aipu_status_t ParserV12::parse_graph_header_check(std::istream &gbin,
                                                  uint32_t gbin_sz) {
  ...
  if (header_sz < GBIN_HEADER_MIN_SZ || header_sz > gbin_sz) {
    LOG(LOG_ERR, "graph bin header sz invalid");
    goto finish;
  }

  if (top_header.file_size < gbin_sz || top_header.file_size > gbin_sz) {
    LOG(LOG_ERR, "graph bin file sz invalid");
    goto finish;
  }

  if (bot_header.entry > gbin_sz) {
    LOG(LOG_ERR, "graph bin entry invalid");
    goto finish;
  }
  ...
  if (((bot_header.text_size > 0) && (tmp < header_sz || tmp > gbin_sz)) ||
    ...
    LOG(LOG_ERR, "graph bin text sz invalid");
    goto finish;
  }
  ...
  if (((bot_header.data_size > 0) && (tmp < header_sz || tmp > gbin_sz)) ||
    ...
    LOG(LOG_ERR, "graph bin data sz invalid");
    goto finish;
  }
  ...
  if (((bot_header.bss_size > 0) && (tmp < header_sz || tmp > gbin_sz)) ||
    ...
    LOG(LOG_ERR, "graph bin bss sz invalid");
    goto finish;
  }
  ...
  if (((bot_header.rodata_size > 0) && (tmp < header_sz || tmp > gbin_sz)) ||
    ...
    LOG(LOG_ERR, "graph bin rodata sz invalid");
    goto finish;
  }
  ...
  if (((bot_header.dcr_size > 0) && (tmp < header_sz || tmp > gbin_sz)) ||
    ...
    LOG(LOG_ERR, "graph bin dcr sz invalid");
    goto finish;
  }
...
aipu_status_t ParserV12::parse_graph_header_bottom(std::istream &gbin,
                                                   Graph &gobj) {
...
aipu_status_t ParserV12::parse_graph(std::istream &gbin, uint32_t size,
                                     Graph &gobj) {
  ...
  gobj.set_graph_text(m_sections[SECTION_TYPE_TEXT].va,
                      m_sections[SECTION_TYPE_TEXT].size);
