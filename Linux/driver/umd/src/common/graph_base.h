...
public:
  ...
  virtual aipu_status_t get_elf_note(const std::string &note_name, char *data,
                                     uint64_t size) {
    return AIPU_STATUS_ERROR_GVERSION_UNSUPPORTED;
  }
...
public:
  ...
  void set_sram_flag(uint32_t sram_flag) { m_sram_flag = sram_flag; }
  void set_remap_flag(uint32_t flag) { m_remap_flag = flag; }
  void set_buildversion(uint32_t buildversion) {
    m_aipubin_buildversion = buildversion;
  }
  void set_disable_input_reuse(bool disable_reuse) {
    m_disable_input_reuse = disable_reuse;
  }
  void set_disable_output_reuse(bool disable_reuse) {
    m_disable_output_reuse = disable_reuse;
  }
