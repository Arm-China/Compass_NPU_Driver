...
void ParserBase::print_graph_header_top(const BinHeaderTop &top) {
#if (defined PRINT_GRAPH_HDR_PARSING_INFO) &&                                  \
    (PRINT_GRAPH_HDR_PARSING_INFO == 1)
  LOG(LOG_DEFAULT, "===========================AIPU Bin Header "
                   "(Top)===========================");
  LOG(LOG_DEFAULT, "Graph magic: %s", top.magic);
  LOG(LOG_DEFAULT,
      "Target device: 0x%x (arch %u, version %u, configuration %u, revision "
      "%u)",
      top.device, AIPU_ARCH(top.device), AIPU_VERSION(top.device),
      AIPU_CONFIG(top.device), AIPU_REVISION(top.device));
  LOG(LOG_DEFAULT, "Graph version: %u", GRAPH_VERSION(top.version));
  LOG(LOG_DEFAULT,
      "Building tool version: 0x%x (major %u, minor %u, build number %u)",
      top.build_version, BUILD_MAJOR(top.build_version),
      BUILD_MINOR(top.build_version), BUILD_NUMBER(top.build_version));
  ...
  LOG(LOG_DEFAULT,
      "Graph flag: 0x%x (ASID %u, ASID_EN %u, REMAP_EN %u, SRAM_EN %u)",
      top.flag, GET_ASID_FLAG(top.flag), IS_ASID_ENABLED(top.flag),
      GET_REMAP_FLAG(top.flag), GET_SRAM_FLAG(top.flag));
  LOG(LOG_DEFAULT, "==========================================================="
                   "================");
#endif
}
...
aipu_status_t
ParserBase::sort_io_tensor(std::vector<GraphIOTensorDesc> &tensors) const {
...
aipu_status_t ParserBase::fill_io_tensor_desc_inner(
  ...
  case SECTION_TYPE_OUT_TENSOR_SHAPE:
    ...
    break;
  default:
    LOG(LOG_WARN, "no sub_section type: %d", sub_section_load.type);
...
aipu_status_t ParserBase::parse_bss_section(char *bss, uint32_t size,
                                            uint32_t bss_id, Graph &gobj,
                                            char **next) const {
  aipu_status_t ret = AIPU_STATUS_SUCCESS;
  ...
  uint32_t cst_indices_size_in_gm = 0, zerocpy_cst_indices_size_in_gm = 0;
  ...
  gobj.set_stack(bss_id, bss_header.stack_size,
                 ALIGN_ADDR(bss_header.stack_align_bytes));

  /* static sections (weight/bias) in bss */
  desc_load_addr = (char *)(bss + sizeof(BSSHeader));
  for (uint32_t static_sec_iter = 0;
       static_sec_iter < bss_header.static_section_desc_cnt;
       static_sec_iter++) {
    section_ir.init();
    if (umd_is_valid_ptr(load_lb, load_ub, desc_load_addr,
                         sizeof(BSSStaticSectionDesc)))
    ...
    for (uint32_t sub_sec_iter = 0;
         sub_sec_iter < static_desc_load.sub_section_cnt; sub_sec_iter++) {
      GraphSubSectionDesc sub_desc_ir;
      if (umd_is_valid_ptr(load_lb, load_ub, desc_load_addr,
                           sizeof(SubSectionDesc)))
      ...
      for (uint32_t ro_entry_iter = 0;
           ro_entry_iter < sub_desc_load.offset_in_ro_cnt; ro_entry_iter++) {
        ...
        if (sub_desc_load.type == SECTION_TYPE_ZEROCPY_CONSTANT)
          LOG(LOG_DEBUG,
              "s %d: static_desc_load.sub_section_cnt = %d, "
              "sub_desc_load.offset_in_ro_cnt=%d, "
              "sub_desc_load.offset_in_section_exec=%d, offset_in_ro=%x",
              static_sec_iter, static_desc_load.sub_section_cnt,
              sub_desc_load.offset_in_ro_cnt,
              sub_desc_load.offset_in_section_exec, offset_in_ro);

        param.init(offset_in_ro, PARAM_MAP_LOAD_TYPE_STATIC, 0,
                   m_static_buf_idx, sub_sec_iter,
                   sub_desc_load.offset_in_section_exec,
                   sub_desc_load.addr_mask);
    ...
    if (gobj.get_swt_idxes().count(m_static_buf_idx) &&
        gobj.get_swt_mem_region() == AIPU_MEM_REGION_GM) {
      if (section_ir.type == SECTION_TYPE_ZEROCPY_CONSTANT) {
        zerocpy_cst_indices_size_in_gm = aligned(zerocpy_cst_indices_size_in_gm,
                                                 static_desc_load.align_bytes) +
                                         section_ir.size;
      } else {
        cst_indices_size_in_gm = ALIGN_PAGE(
            aligned(cst_indices_size_in_gm, static_desc_load.align_bytes) +
            section_ir.size);
      }
    } else {
  ...
  gobj.set_weight_indices_size(bss_id, cst_indices_size_in_gm,
  ...
  for (uint32_t reuse_sec_iter = 0;
       reuse_sec_iter < bss_header.reuse_section_desc_cnt; reuse_sec_iter++) {
    section_ir.init();
    if (umd_is_valid_ptr(load_lb, load_ub, desc_load_addr,
                         sizeof(BSSReuseSectionDesc)))
    ...
    for (uint32_t sub_sec_iter = 0;
         sub_sec_iter < reuse_desc_load.sub_section_cnt; sub_sec_iter++) {
      GraphSubSectionDesc sub_desc_ir;
      if (umd_is_valid_ptr(load_lb, load_ub, desc_load_addr,
                           sizeof(SubSectionDesc)))
        memcpy(&sub_desc_load, desc_load_addr, sizeof(SubSectionDesc));
      ...
      if ((sub_desc_load.type == SECTION_TYPE_INPUT) ||
        ...
        fill_io_tensor_desc_inner<SubSectionDesc>(m_reuse_buf_idx, sub_sec_iter,
                                                  sub_desc_load, io);
      }
      ...
      for (uint32_t ro_entry_iter = 0;
        ...
        param.init(offset_in_ro, PARAM_MAP_LOAD_TYPE_REUSE, sub_desc_load.type,
                   m_reuse_buf_idx, sub_sec_iter,
                   sub_desc_load.offset_in_section_exec,
                   sub_desc_load.addr_mask);
        gobj.add_param(0, param);
        desc_load_addr = (char *)(desc_load_addr + sizeof(uint32_t));
...
overflow:
  LOG(LOG_DEBUG,
      "[UMD ERROR] Input graph binary contains invalid offset value(s)!");
  ret = AIPU_STATUS_ERROR_INVALID_GBIN;
...
aipu_status_t ParserBase::parse_remap_section(char *remap, Graph &gobj) {
  aipu_status_t ret = AIPU_STATUS_SUCCESS;
...
aipu_status_t ParserBase::parse_graph_header_top(std::istream &gbin,
                                                 uint32_t size, Graph &gobj) {
  aipu_status_t ret = AIPU_STATUS_SUCCESS;
...
uint32_t ParserBase::get_graph_bin_version(std::istream &gbin) {
