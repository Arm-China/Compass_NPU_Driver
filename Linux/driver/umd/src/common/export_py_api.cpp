...
struct aipu_load_graph_cfg_wrapper_t {
  uint8_t wt_mem_region = 0;
  std::vector<int32_t> wt_idxes;
...
struct aipu_dynshape_dim_num_wrapper_t {
  ...
  std::vector<uint32_t>
      ds_dim_num; /**< the dynamic dim number of requested shape index */
};

struct aipu_dynshape_info_wrapper_t {
  uint64_t graph_id;   /**< the graph id to be searched */
  uint32_t ds_idx;     /**< the dynamic shape index */
  bool max_threshhold; /**< true: for max shape, false: for min shape */
  std::vector<uint32_t>
      ds_data; /**< the dynamic shape information of inquired */
};
...
public:
  ...
  py::tuple aipu_load_graph_py(const std::string &graph_bin,
    ...
    if (!cfg.is_none()) {
      ...
      aipu_load_graph_cfg_t cfg_;
      memset(&cfg_, 0, sizeof(aipu_load_graph_cfg_t));
      cfg_.wt_mem_region = wrapper.wt_mem_region;
      if (wrapper.wt_idxes.size() != 0) {
        cfg_.wt_idxes_cnt = wrapper.wt_idxes.size();
        cfg_.wt_idxes = const_cast<int32_t *>(wrapper.wt_idxes.data());
      }
      if (!wrapper.extra_weight_path.empty())
        cfg_.extra_weight_path = wrapper.extra_weight_path.c_str();

      ret = aipu_load_graph(m_ctx, graph_bin.c_str(), &graph_id, &cfg_);
    } else {
      ret = aipu_load_graph(m_ctx, graph_bin.c_str(), &graph_id, nullptr);
    }
  ...
  py::tuple aipu_load_graph_helper_py(py::handle graph_buffer,
    ...
    if (!cfg.is_none()) {
      ...
      ret = aipu_load_graph_helper(m_ctx, data, graph_size, &graph_id, &cfg_);
    } else {
      ret = aipu_load_graph_helper(m_ctx, data, graph_size, &graph_id, nullptr);
    }
  ...
  py::tuple aipu_create_job_py(uint64_t graph_id, const py::handle &cfg) {
    ...
    if (!cfg.is_none()) {
      ...
      aipu_create_job_cfg_t cfg_;
      memset(&cfg_, 0, sizeof(aipu_create_job_cfg_t));
  ...
  aipu_status_t aipu_config_job_py(uint64_t job_id, uint64_t types,
    ...
    if (types == AIPU_CONFIG_TYPE_SIMULATION) {
      ...
      aipu_job_config_simulation_t cfg_;
   ...
   *               TODO: details py::arg to data struct
   * @retval     tuple
   *              - AIPU_IOCTL_EN_TICK_COUNTER
   *                . aipu_status_t: AIPU_STATUS_SUCCESS if success
   *              - AIPU_IOCTL_DI_TICK_COUNTER
   *                . aipu_status_t: AIPU_STATUS_SUCCESS if success
   ...
   *               . py::bytes
   */
  py::tuple aipu_ioctl_py(uint32_t cmd,
    ...
    case AIPU_IOCTL_EN_TICK_COUNTER:
      cmd = AIPU_IOCTL_ENABLE_TICKCOUNTER;
      break;
    ...
    case AIPU_IOCTL_ALLOC_DMABUF: {
      ...
      arg = malloc(sizeof(aipu_dma_buf_req_t));
      aipu_dma_buf_req_t *p = (aipu_dma_buf_req_t *)arg;
      memset(p, 0, sizeof(aipu_dma_buf_req_t));
      if (py_arg.count(str_key[0]) == 1)
    ...
    if (cmd == AIPU_IOCTL_READ_DMABUF) {
      ...
      for (uint32_t i = 0; i < p->size; i++)
        ret_tuple = py::make_tuple(ret, py::bytes(p->data, p->size));

      delete[] p->data;
      p->data = nullptr;
    } else if (cmd == AIPU_IOCTL_ALLOC_DMABUF) {
      aipu_dma_buf_req_t *p = (aipu_dma_buf_req_t *)arg;
      ret_tuple = py::make_tuple(ret, p->fd);
    } else if (cmd == AIPU_IOCTL_ALLOC_SHARE_BUF) {
      aipu_share_buf_t *p = (aipu_share_buf_t *)arg;
      ret_tuple = py::make_tuple(ret, p->pa, p->va);
    } else {
      ret_tuple = py::make_tuple(ret);
    }
  ...
  /**
   * @brief This API is used to send specific dma buffer to NPU driver.
   *
   * @param[in] cmd cmd: AIPU_IOCTL_WRITE_DMABUF
   * @param[in] py_arg input argument according to 'cmd'
   *            - "dmabuf_fd":
   *            - "offset_in_dmabuf":
   *            - "size":
   * @param[in] data input for dma buffer
   *
   * @retval AIPU_STATUS_SUCCESS if success
   */
  aipu_status_t aipu_ioctl_write_dmabuf_py(
      uint32_t cmd, std::map<std::string, uint64_t> py_arg, char *data) {
    ...
    if (cmd != AIPU_IOCTL_WRITE_DMABUF) {
      fprintf(stderr, "[PY UMD ERROR] aipu_ioctl with 3 args must use cmd: "
   ...
   * @retval AIPU_STATUS_SUCCESS if success
   */
  aipu_status_t aipu_specify_iobuf_py(uint64_t job_id,
                                      std::map<std::string, uint64_t> py_arg) {
    aipu_status_t ret = AIPU_STATUS_SUCCESS;
    const char *status_msg = nullptr;
    std::string str_key[] = {"type",
                             "tensor_idx",
                             "id",
                             "pa",
                             "dmabuf_fd",
...
PYBIND11_MODULE(libaipudrv, m) {
  ...
  py::class_<aipu_dynshape_num_wrapper_t>(m, "aipu_dynshape_num_t")
      .def(py::init<>())
      .def_readwrite("graph_id", &aipu_dynshape_num_wrapper_t::graph_id)
      .def_readwrite("ds_num", &aipu_dynshape_num_wrapper_t::ds_num);

  py::class_<aipu_dynshape_dim_num_wrapper_t>(m, "aipu_dynshape_dim_num_t")
      .def(py::init<>())
      .def_readwrite("graph_id", &aipu_dynshape_dim_num_wrapper_t::graph_id)
      .def_readwrite("ds_idx", &aipu_dynshape_dim_num_wrapper_t::ds_idx)
      .def_readwrite("max_threshhold",
                     &aipu_dynshape_dim_num_wrapper_t::max_threshhold)
      .def_readwrite("ds_dim_num",
                     &aipu_dynshape_dim_num_wrapper_t::ds_dim_num);

  py::class_<aipu_dynshape_info_wrapper_t>(m, "aipu_dynshape_info_t")
      .def(py::init<>())
      .def_readwrite("graph_id", &aipu_dynshape_info_wrapper_t::graph_id)
      .def_readwrite("ds_idx", &aipu_dynshape_info_wrapper_t::ds_idx)
      .def_readwrite("max_threshhold",
                     &aipu_dynshape_info_wrapper_t::max_threshhold)
      .def_readwrite("ds_data", &aipu_dynshape_info_wrapper_t::ds_data);
  ...
  py::enum_<aipu_ioctl_cmd_t>(m, "aipu_ioctl_cmd_t")
      ...
      .value("AIPU_IOCTL_GET_DS_DIM_NUM",
             aipu_ioctl_cmd_t::AIPU_IOCTL_GET_DS_DIM_NUM)
  ...
  py::enum_<aipu_ioctl_tickcounter_t>(m, "aipu_ioctl_tickcounter_t")
      .value("AIPU_IOCTL_EN_TICK_COUNTER",
             aipu_ioctl_tickcounter_t::AIPU_IOCTL_EN_TICK_COUNTER)
      .value("AIPU_IOCTL_DI_TICK_COUNTER",
             aipu_ioctl_tickcounter_t::AIPU_IOCTL_DI_TICK_COUNTER)
      .export_values();
  ...
  py::class_<NPU>(m, "NPU")
      ...
      .def("aipu_ioctl", &NPU::aipu_ioctl_write_dmabuf_py, py::arg("cmd"),
           py::arg("py_arg"), py::arg("data"))
