...
#define AIPU_DISABLE_INPUT_REUSE(reserved) ((reserved)&0x01)
#define AIPU_DISABLE_OUTPUT_REUSE(reserved) ((reserved >> 1) & 0x01)
...
struct BSSHeader {
  ...
  uint32_t stack_align_bytes; /**< static base address align requirement (in
                                 bytes) */
 ...
 *      8:15	Identify which segment [0-3] are available, [4-255] are
 * reserved. 0:7	    Identify which control [0-1] are available,  value
 * [2,255] are reserved. and addr_mask is also used as mask for rewrite segmmu
 * controls.
 */
struct SubSectionDesc {
  uint32_t offset_in_section_exec; /**< offset in a section */
  uint32_t size;                   /**< tensor size */
  uint32_t type;        /**< tensor type: 0: input; 1: output; 2: saved/dump; 3:
                           intermediate */
  uint32_t id;          /**< tensor ID, used for identifying different
                           inputs/outputs/segmmus */
  uint32_t data_type;   /**< tensor layout type: 0: none; 1: bool; 2: uint8; 3:
                           int8; 4: uint16; 5: int16; */
  float scale;          /**< scale */
  int32_t zero_point;   /**< zero point */
  uint32_t reserved[3]; /**< reserved unused */
  uint32_t addr_mask; /**< sub-section addr mask, ro_new = (addr & addr_mask) |
                         (ro_old & ~addr_mask) */
  uint32_t offset_in_ro_cnt; /**< number of offset(s) in rodata section where
                                sub-section base address should be loaded */
};
...
struct BSSStaticSectionDesc {
  uint32_t src_offset; /**< static data offset in loadable graph file */
  uint32_t size;       /**< section size */
  uint32_t
      align_bytes; /**< section base address align requirement (in bytes) */
...
public:
  ...
  static aipu_status_t parse_graph_header_top(std::istream &gbin, uint32_t size,
                                              Graph &gobj);
