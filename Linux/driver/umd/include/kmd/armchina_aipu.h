 ...
 *
 * Structures defined in this header are shared by all hardware versions, but
 * some fields of the structs are specific to certain hardware version(s).
 *
 * In the following member descriptions,
 * [must]     mean that the fields must be filled by user mode driver in all
 * cases. [optional] mean that the fields is optional for user mode driver to
 * enable or not. [alloc]    mean that the buffer(s) represented by the fields
 * must be allocated by user mode driver before calling IOCTL. [kmd]      mean
 * that the fields should be filled by kernel mode driver if the calls are
 * successful. [only]     mean that the fields are only for the hardware
 * versions specified to use.
 */
 ...
 *
 *     For aipu v1/v2, a *partition* represents an AIPU core, and the partition
 * count is the count of AIPU cores in current system. For aipu v3, a
 * *partition* represents a group of clusters in the same power/computation
 *         domain, and the partition count is the count of such cluster groups.
 * All partitions share the same TSM and external register base address.
 */

/**
 * struct aipu_config_clusters - struct used to configure the AIPU clusters (v3
 * only)
 *
 ...
 * If the core count is 1, the per-core capability is in the partition_cap
 * member; otherwise user mode driver should get all the per-core capabilities
 * as the partition_cnt indicates via AIPU_IOCTL_QUERY_PARTITION_CAP command.
 *
 * For aipu v3, user mode driver should get the cluster counts by
 * AIPU_IOCTL_QUERY_PARTITION_CAP command if partition_cnt > 1.
 */
struct aipu_cap {
  __u32 partition_cnt;
  __u32 asid_cnt;
 ...
 *                        KMD returns this info. to the requesting user thread
 * in one ioctl
 ...
 * @region:        [kmd] set to request a buffer in a default DDR region or a GM
 * region
 * @asid:          [aipu v2/v3 only, optional] from which region (ASID 0/1/2/3)
 * to request
 ...
 * @AIPU_JOB_EXEC_FLAG_QOS_SLOW:     [aipu v3 only] Quality of Service (QoS)
 * slow
 * @AIPU_JOB_EXEC_FLAG_QOS_FAST:     [aipu v3 only] QoS fast
 * @AIPU_JOB_EXEC_FLAG_SINGLE_GROUP: [aipu v3 only] the scheduled job is a
 * single group task
 * @AIPU_JOB_EXEC_FLAG_MULTI_GROUP:  [aipu v3 only] the scheduled job is a
 * multi-groups task
 * @AIPU_JOB_EXEC_FLAG_DBG_DISPATCH: [aipu v3 only] the job should be scheduled
 * with debug-dispatch
 * @AIPU_JOB_EXEC_FLAG_SEG_MMU:      [aipu v3 only] the job has configured
 * segment mmu
 */
 ...
 * @is_defer_run:      [aipu v1/v2 only, optional] Reserve a core for this job
 * and defer to run
 * @version_compatible:[aipu v1/v2 only, optional] Is this job compatible on
 * different ISA versions
 * @core_id:           [aipu v1/v2/v3 optional] ID of the core to reserve/debug
 * dispatch (v3)
 * @partition_id:      [aipu v3 must] ID of the partition requested to schedule
 * a job onto
 * @do_trigger:        [aipu v1/v2 only, optional] Trigger the deferred job to
 * run
 ...
 * @start_pc_addr:     [aipu v1/v2 only, must] Address of the start PC (buf_pa -
 * asid_base)
 * @intr_handler_addr: [aipu v1/v2 only, must] Address of the interrupt handler
 * (pa - asid_base)
 * @data_0_addr:       [aipu v1/v2 only, must] Address of the 0th data buffer
 * (buf_pa - asid_base)
 * @data_1_addr:       [aipu v1/v2 only, must] Address of the 1th data buffer
 * (buf_pa - asid_base)
 * @job_id:            [aipu v1/v2 only, must] ID of this job
 * @enable_prof:       [aipu v1/v2 only, optional] Enable performance profiling
 * counters in SoC
 * @profile_pa:        [optional] Physical address of the profiler buffer
 * @profile_sz:        [optional] Size of the profiler buffer (should be 0 if no
 * such a buffer)
 * @profile_fd:        [aipu v3 only] Profile data file fd
 * @enable_poll_opt:   [aipu v1/v2 only, optional] Enable optimizations for job
 * status polling
 * @exec_flag:         [optional] Combinations of execution flags
 * @dtcm_size_kb:      [aipu v2(x1)only, optional] DTCM size in KB
 * @head_tcb_pa:       [aipu v3 only, must] base address of the first init TCB
 * of this job
 * @first_task_tcb_pa: [aipu v3 only, must] base address of the first task TCB
 * of this job
 * @last_task_tcb_pa:  [aipu v3 only, must] base address of the last task TCB of
 * this job
 * @tail_tcb_pa:       [aipu v3 only, must] base address of the tail TCB of this
 * job
 * @is_coredump_en:    [aipu v3 and above only, optional] Coredump is enable or
 * not
 *
 ...
 * @status:         [alloc] Pointer to an array (length is max_cnt) to store the
 * status
 ...
 *
 * UMD only need to call this when the partition count returned by
 * AIPU_IOCTL_QUERYCAP > 1.
 */
#define AIPU_IOCTL_QUERY_PARTITION_CAP                                         \
  _IOR(AIPU_IOCTL_MAGIC, 1, struct aipu_partition_cap)
/**
 ...
 *
 * This fails if kernel driver cannot find a free buffer meets the
 ...
 *
 * This fails if the there is no SRAM in the system or the SRAM has already been
 * allocated.
 */
 ...
 *
 * This is a non-blocking operation therefore user mode driver should check the
 * job status via AIPU_IOCTL_QUERY_STATUS.
 */
 ...
 */
#define AIPU_IOCTL_QUERY_STATUS                                                \
  _IOWR(AIPU_IOCTL_MAGIC, 7, struct aipu_job_status_query)
/**
 * DOC: AIPU_IOCTL_KILL_TIMEOUT_JOB
 *
 * @Description
 *
 * ioctl to kill a timeout job and clean it from kernel mode driver; works for
 * aipu v1/v2 only.
 */
 ...
 * @Description
 *
 * ioctl to read/write an external register of an AIPU core; works for aipu
 * v1/v2 only.
 */
 ...
 */
#define AIPU_IOCTL_GET_HW_STATUS                                               \
  _IOR(AIPU_IOCTL_MAGIC, 10, struct aipu_hw_status)
/**
 ...
 */
#define AIPU_IOCTL_CONFIG_CLUSTERS                                             \
  _IOW(AIPU_IOCTL_MAGIC, 14, struct aipu_config_clusters)
/**
 ...
 *
 */
#define AIPU_IOCTL_ALLOC_DMA_BUF                                               \
  _IOR(AIPU_IOCTL_MAGIC, 15, struct aipu_dma_buf_request)
/**
 ...
 */
#define AIPU_IOCTL_GET_DMA_BUF_INFO                                            \
  _IOWR(AIPU_IOCTL_MAGIC, 17, struct aipu_dma_buf)
/**
 ...
 *   aipu_dma_buf->bytes: filled by KMD
 */
#define AIPU_IOCTL_ATTACH_DMA_BUF                                              \
  _IOWR(AIPU_IOCTL_MAGIC, 19, struct aipu_dma_buf)
/**
 ...
 */
#define AIPU_IOCTL_ALLOC_GROUP_ID                                              \
  _IOWR(AIPU_IOCTL_MAGIC, 22, struct aipu_group_id_desc)
/**
 ...
 */
#define AIPU_IOCTL_FREE_GROUP_ID                                               \
  _IOW(AIPU_IOCTL_MAGIC, 23, struct aipu_group_id_desc)
