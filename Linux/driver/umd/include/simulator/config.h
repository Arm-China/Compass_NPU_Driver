...
enum
{
    ...
    PERF_MODE_SLOW,     //!< bit accurate with profile.
};
...
struct config_t
{
    ...
    bool print_subg_info;               //!< prints subgraph information, when graph_filename is set.
    int8_t fp_mode;                     //!< x3 only, floating-point model: 0 for softfloat (default), 1 for cpu naive float
    struct {
        ...
        struct {
            ...
            uint32_t bandwidth;      //!< fast evaluation: ddr bandwidth setting, indicate how may bits of the axi data bus
        } ddr;
        union {
            struct{
                uint8_t event_size;
