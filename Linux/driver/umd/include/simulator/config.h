...
@note enable_avx is use the intel AVX instruction to speed up, but it is not
yet.
*/
struct config_t {
  enum {
  ...
  bool disable_eval = 0; //!< disable en_eval when set, only for Z2/Z3/X1/X2>
  ...
  bool en_idu_perf = 0;  //!< idu issue perf eval.
  bool non_vliw_mode =
      0;            //!< run on non-vliw mode. only effective when en_idu_perf=1
  ...
  uint32_t ddr_bw = 256; //!< fast evaluation: ddr bandwidth setting, indicate
  ...
  std::vector<uint8_t>
      isa_events; // isa events to be profiled, default is None.
  bool print_json =
      0; // For X3: used to print json subgraph when SPM not enabled.
};
